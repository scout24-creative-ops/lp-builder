...
- Use `choice-card-expand-list` when comparable choice cards need optional, expandable feature details; follow §10.10c exactly.
...
Use `choice-card-expand-list` for 2–4 comparable options when the feature details should remain collapsible. On desktop, the open/closed state is synchronized across all cards; on mobile, each card can be expanded independently. Use the exact component-library markup and Guardrails §10.10c.
